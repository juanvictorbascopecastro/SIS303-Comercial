...
class VentasModel extends Model {
    ...
    public function get(){
        try{
            $result = $this->db->connect()->query("SELECT v.idventa, c.idcliente, u.idusuario, v.fecha, v.detalles, c.dni, CONCAT(c.nombre, ' ', c.apellido) AS cliente, CONCAT(u.nombre, ' ', u.apellido) AS usuario, u.rol, v.total FROM ventas v INNER JOIN view_cliente c ON c.idcliente = v.idcliente INNER JOIN view_usuarios u ON u.idusuario = v.idusuario ORDER BY v.fecha DESC LIMIT 100");          
            $result->execute();
    ...
    public function search($data){
        try{
            $statement = $this->db->connect()->prepare("SELECT v.idventa, c.idcliente, u.idusuario, v.fecha, v.detalles, c.dni, 
            CONCAT(c.nombre, ' ', c.apellido) AS cliente, CONCAT(u.nombre, ' ', u.apellido) AS usuario, u.rol, v.total 
            FROM ventas v INNER JOIN view_cliente c ON c.idcliente = v.idcliente INNER JOIN view_usuarios u ON u.idusuario = v.idusuario 
    ...
    public function id($idventa){
        try{
            $statement = $this->db->connect()->prepare('SELECT p.idproducto, p.idcategoria, p.nombre, p.descripcion, p.imagen, p.precio, pv.idventa_producto, pv.cantidad FROM producto p INNER JOIN producto_venta pv ON p.idproducto = pv.idproducto WHERE pv.idventa = :idventa');  
